...
contract CoboFundVaultTest is FundTestBase {
    ...
    function test_vault_initialize_normal() public view {
        ...
        assertEq(asset.allowance(address(vault), address(fundToken)), type(uint256).max);
    }
    ...
    function test_vault_initialize_revert_zeroNav4626() public {
        CoboFundVault impl = new CoboFundVault();
        vm.expectRevert(LibFundErrors.ZeroAddress.selector);
        new ERC1967Proxy(address(impl), abi.encodeCall(CoboFundVault.initialize, (address(asset), address(0), admin)));
    }
    ...
    function test_vault_setFundToken() public {
        ...
        assertEq(asset.allowance(address(vault), newNav), type(uint256).max);
    }
    ...
    function test_vault_rescueERC20_revert_coreAsset() public {
        asset.mint(address(vault), 100e6);
        vm.prank(admin);
        vm.expectRevert(abi.encodeWithSelector(LibFundErrors.CannotRescueCoreAsset.selector, address(asset)));
        vault.rescueERC20(address(asset), admin, 100e6);
    }
    ...
    /// @dev V-WD-2: Verify ASSET balance changes (vault -amount, recipient +amount)
    function test_vault_withdraw_balanceChanges() public {
        ...
        assertEq(asset.balanceOf(address(vault)), vaultBefore - 60e6);
        assertEq(asset.balanceOf(user1), user1Before + 60e6);
    }
    ...
    /// @dev V-WD-8: Insufficient vault ASSET balance reverts
    ...
    function test_vault_setFundToken_oldCannotTransferFrom() public {
        ...
        // Old Nav4626 tries to pull ASSET from vault — should fail (allowance == 0)
        vm.prank(oldNav);
        vm.expectRevert();
        asset.transferFrom(address(vault), oldNav, 1e6);
    }
    ...
    function test_vault_setFundToken_newCanTransferFrom() public {
        ...
        // New Nav4626 pulls ASSET from vault — should succeed (allowance == max)
        vm.prank(newNav);
        asset.transferFrom(address(vault), newNav, 1e6);

        assertEq(asset.balanceOf(newNav), 1e6);
    }
    ...
    function test_vault_setFundToken_newPaused_withdrawReverts() public {
        ...
        CoboFundVault dummyVault = CoboFundVault(
            address(
                new ERC1967Proxy(
                    address(vaultImpl),
                    abi.encodeCall(CoboFundVault.initialize, (address(asset), address(newNav4626), admin))
                )
            )
        );
    ...
    function test_vault_grantSettlementOperator_newOperatorCanWithdraw() public {
        ...
        vault.withdraw(user1, 10e6);
        assertEq(asset.balanceOf(user1), 1010e6); // 1000 initial + 10
    }
    ...
    function test_vault_settlementOperator_newOperatorWithdrawFlow() public {
        ...
        assertEq(asset.balanceOf(user1), 1050e6);
        assertEq(asset.balanceOf(user2), 1050e6);
    }
